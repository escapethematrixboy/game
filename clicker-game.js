...
// Save filename
var SAVE_FILE = 'clicker-save.json';
...
function getBuildingProduction(building) {
    var multiplier = 1;
    for (var _i = 0, _a = building.upgrades; _i < _a.length; _i++) {
        var upgrade = _a[_i];
...
function getTotalProduction() {
    return gameState.buildings.reduce(function (total, building) {
...
function buyBuilding(index) {
    var building = gameState.buildings[index];
...
// Buy an upgrade
function buyUpgrade(buildingIndex, upgradeIndex) {
    var building = gameState.buildings[buildingIndex];
    var upgrade = building.upgrades[upgradeIndex];
...
function displayStatus() {
    ...
    console.log("Points: ".concat(formatNumber(gameState.points), " (").concat(formatNumber(getTotalProduction()), " per second)"));
    console.log("Total earned: ".concat(formatNumber(gameState.totalPointsEarned)));
    console.log('\n=== BUILDINGS ===');
    gameState.buildings.forEach(function (building, i) {
        console.log("".concat(i + 1, ". ").concat(building.name, ": ").concat(building.count, " | Cost: ").concat(formatNumber(getBuildingCost(building)), " points | Producing: ").concat(formatNumber(getBuildingProduction(building)), " per second"));
    });
    console.log('\n=== UPGRADES ===');
    gameState.buildings.forEach(function (building, i) {
        building.upgrades.forEach(function (upgrade, j) {
            if (!upgrade.purchased) {
                console.log("".concat(i + 1).concat(String.fromCharCode(97 + j), ". ").concat(building.name, " - ").concat(upgrade.name, ": ").concat(formatNumber(upgrade.cost), " points | ").concat(upgrade.description));
            }
        });
    });
    console.log('\n=== COMMANDS ===');
...
function gameLoop() {
    var now = Date.now();
    var elapsed = (now - lastUpdate) / 1000; // seconds
    lastUpdate = now;
    // Add points from passive production
    var pointsGained = getTotalProduction() * elapsed;
...
function handleInput(input) {
    ...
    switch (input) {
        ...
        case '2':
        case '3':
            var buildingIndex = parseInt(input) - 1;
            if (buyBuilding(buildingIndex)) {
                console.log("Purchased a ".concat(gameState.buildings[buildingIndex].name, "!"));
            }
            else {
